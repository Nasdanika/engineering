...
public class ModelElementViewActionImpl<T extends ModelElement> extends SimpleEObjectViewAction<T> implements ModelElementViewAction<T> {
	...
	protected Object memberValue(ETypedElement member, Object value, ViewGenerator viewGenerator, ProgressMonitor progressMonitor) {	
		if (value instanceof Temporal) {
			return temporalValue(member, (Temporal) value, viewGenerator, progressMonitor, true);
		}
	...
	protected Object temporalValue(ETypedElement member, Temporal temporal, ViewGenerator viewGenerator, ProgressMonitor progressMonitor, boolean withModalTrigger) {
		...
		if ((offset == null || offset.equals(Duration.ZERO)) && base != null) {
			return "Coincides with " 
					+ temporalValue(member, base, viewGenerator, progressMonitor, withModalTrigger)
	...
	protected Object temporalInfoModalTrigger(ETypedElement member, Temporal temporal, ViewGenerator viewGenerator, ProgressMonitor progressMonitor) {
		...
		if (modalContent != null) {
			...
			Tag modalTitle = viewGenerator.getHTMLFactory().tag(TagName.h5, TagName.span.create().addClass(questionCircleIcon).style().margin().right("0.3em"), temporalValue(member, temporal, viewGenerator, progressMonitor, false));
