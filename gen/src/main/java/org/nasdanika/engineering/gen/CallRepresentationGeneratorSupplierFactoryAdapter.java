...
public class CallRepresentationGeneratorSupplierFactoryAdapter extends AdapterImpl implements RepresentationGenerator.Supplier.Factory {

	public CallRepresentationGeneratorSupplierFactoryAdapter(Call target) {
		setTarget(target);
	}
	
	@Override
	public boolean isAdapterForType(Object type) {
		return type == RepresentationGenerator.Supplier.Factory.class;
	}
	...
	protected Function<Map<EStructuralFeature,Object>, RepresentationGenerator> createResultFunction(Context context) {
		Call call = (Call) getTarget();
		
		return new Function<Map<EStructuralFeature,Object>, RepresentationGenerator>() {
			...
			public RepresentationGenerator execute(Map<EStructuralFeature,Object> data, ProgressMonitor progressMonitor) {
				...
				if (result instanceof RepresentationGenerator) {
					return (RepresentationGenerator) result;
				}

				throw new ConfigurationException("Result is not an instance of RepresentationGenerator: " + result, EObjectAdaptable.adaptTo((EObject) getTarget(), Marked.class));
			}
		};
	}
